...
# --- 新增：兼容本地外部测试的DLL路径查找逻辑 ---
if not global_dll_path.exists():
    # 尝试指向您真实的通达信安装目录
    fallback_path = Path(r'D:\new_tdx64\PYPlugins\TPythClient.dll')
    if not fallback_path.exists():
        fallback_path = Path(r'D:\new_tdx64\T0002\PYPlugins\TPythClient.dll') # 备选路径
    
    if fallback_path.exists():
        global_dll_path = fallback_path

if hasattr(os, 'add_dll_directory') and global_dll_path.exists():
    os.add_dll_directory(str(global_dll_path.parent))
    os.add_dll_directory(r'D:\new_tdx64') # 兼容通达信根目录的依赖DLL
