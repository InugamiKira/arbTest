    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('status') == 'error':
                ...
                print(f"IB夜盘价格: GLD=${ib_night_prices.get('GLD', {}).get('bid', 0):.2f}, USO=${ib_night_prices.get('USO', {}).get('bid', 0):.2f}, XOP=${ib_night_prices.get('XOP', {}).get('bid', 0):.2f}")
        else:
...
def generate_fund_data(fund, data_processor, html_generator, futures_data, futures_history_df=None, is_index_table=False, gold_calibration=10.9067, oil_calibration=0.8227, basic_df=None):
    """处理单个基金的数据"""
    code = fund.get('code', '')
    ...
    # 读取基金数据
    lof_df = data_processor.read_lof_data(code)
    ...
    if f_list:
        ...
        if category == '黄金': future_symbol = 'GC'
            ...
            if code in ['161130', '161128']: future_symbol = 'NQ'
            elif code in ['161125', '161127']: future_symbol = 'ES'
    ...
    for i in range(len(sub)):
        ...
        # 获取基金净值
        n_T = df_idx.loc[d_T].get('nav', 0)
        n_T1 = df_idx.loc[d_T1].get('nav', 0) if d_T1 else 0
        n_T2 = df_idx.loc[d_T2].get('nav', 0) if d_T2 else 0
        
        # 获取收盘价
        c_T = df_idx.loc[d_T].get('close', 0)
        # 检查c_T是否为NaN或空值
        if pd.isna(c_T) or c_T == '':
            c_T = 0
        ...
        colspan_main = 13 + len(etf_columns)
    ...
    if not lof_df_sorted.empty:
        ...
        if est_home == 0:
            ...
            for _, row in lof_df_sorted.iterrows():
                ...
                if isinstance(val, (int, float)) and val > 0:
                    # 检查是否有必要的ETF数据
                    has_necessary_etf_data = True
                    if category == '纯ETF' and len(h_list) == 1:
                        # 对于纯ETF类别，只需要检查其主要对冲ETF是否存在
                        symbol = h_list[0]['symbol']
                        if symbol in row:
                            etf_price = row.get(symbol, 0)
                            if pd.isna(etf_price) or etf_price <= 0:
                                has_necessary_etf_data = False
                        else:
                            has_necessary_etf_data = False
                    else:
                        # 对于其他类别，需要检查所有ETF数据
                    ...
                    # 只有当有必要的ETF数据时，才使用静态官方估值
                    if has_necessary_etf_data:
                        valid_estimates.append(val)
                        # 同时更新est_home_date
                        try:
                            est_home_date = row['date'].strftime('%m-%d')
                        except Exception:
                            est_home_date = str(row['date'])[-5:]
                        break
            if valid_estimates:
        ...
        for _, row in lof_df_sorted.iterrows():
            if row.get('nav', 0) > 0 and pd.notna(row.get('nav')):
                ...
                for item in h_list:
                    ...
                    if val > 0:
                        if weight > 0:
                            etf_texts.append(f"{sym}: {val:.2f} 权重 {weight:.1f}%")
                        else:
                            etf_texts.append(f"{sym}: {val:.2f}")
                base_etfs_text = " | ".join(etf_texts)
                
                # 新增：提取期货基准价供 Sandbox 验算使用 - 从 basic_df 读取
                if future_symbol and not basic_df.empty:
                    settle_col = f'{future_symbol}_settle'
                    # 查找匹配日期的行
                    for _, row in basic_df.iterrows():
                        row_date = row.get('date')
                        if pd.isna(row_date):
                            continue
                        row_date_str = row_date.strftime('%Y-%m-%d') if isinstance(row_date, pd.Timestamp) else str(row_date)[:10]
                        if row_date_str == rt_base_date_str:
                            val = row.get(settle_col)
                            if pd.notna(val) and val != '':
                                base_future_price = float(val)
                            break
        ...
        def get_three_hedge_calculations_with_trade():
            # 判断是否有期货对冲相关区域 (针对纯ETF及未配置期货的品类隐藏)
...
def check_and_update_historical_data():
    ...
    for fund in cfg.get('funds', []):
        ...
        # 构建历史数据文件路径
        filename = f"LOF_{code}_history.csv"
        file_path = os.path.join(DATA_DIR, filename)
        
        # 检查文件是否存在
        if not os.path.exists(file_path):
            print(f"警告: 基金 {code} 的历史数据文件不存在，需要更新")
            need_update = True
            break
    ...
    if need_update:
        ...
        try:
            ...
            # 执行生成基础数据的脚本
            print("执行 LOF011_generate_basic_data.py...")
            subprocess.run([sys.executable, "-X", "utf8", os.path.join(SCRIPT_DIR, "LOF011_generate_basic_data.py")], 
                         check=True, capture_output=True, text=True, encoding="utf-8", env=env)
            
            print("执行 LOF012_generate_lof_data.py...")
            subprocess.run([sys.executable, "-X", "utf8", os.path.join(SCRIPT_DIR, "LOF012_generate_lof_data.py")], 
                         check=True, capture_output=True, text=True, encoding="utf-8", env=env)
            
            print("成功: 基础数据与LOF历史数据更新成功")
...
def generate(futures_data=None, ib_data=None):
    ...
    # 从basic表格中获取最新的校准值
    gold_calibration = 10.9067  # 默认值
    oil_calibration = 0.8227  # 默认值
    if not basic_df.empty:
        latest_row = basic_df.iloc[0]
        if '黄金校准' in latest_row:
            gold_calib_val = latest_row['黄金校准']
            if pd.notna(gold_calib_val) and gold_calib_val > 0:
                gold_calibration = float(gold_calib_val)
        if '原油校准' in latest_row:
            oil_calib_val = latest_row['原油校准']
            if pd.notna(oil_calib_val) and oil_calib_val > 0:
                oil_calibration = float(oil_calib_val)
    ...
    for fund in cfg.get('funds', []):
        ...
        lof_df = data_processor.read_lof_data(code)
        base_date, base_nav, base_row = data_processor.get_base_date_info(lof_df)
        if base_date and base_nav:
            ...
            for item in hedging_portfolio:
                sym = item['symbol']
                price = 0.0
                if sym in base_row and not pd.isna(base_row[sym]):
                ...
                if price <= 0:
                    ...
                    if base_sym in base_row and not pd.isna(base_row[base_sym]):
                        price = float(base_row[base_sym])
                    elif f"^{base_sym}" in base_row and not pd.isna(base_row[f"^{base_sym}"]):
                        price = float(base_row[f"^{base_sym}"])
                    # 对于XBI，尝试从其他行获取价格
                    elif base_sym == 'XBI':
                        # 遍历历史数据，找到最近的XBI价格
                        for _, row in lof_df.iterrows():
                            if 'XBI' in row and not pd.isna(row['XBI']) and float(row['XBI']) > 0:
                                price = float(row['XBI'])
                                break
            ...
            if f_list:
                ...
                if category == '黄金': future_symbol_js = 'GC'
                    ...
                    if code in ['161130', '161128']: future_symbol_js = 'NQ'
                    elif code in ['161125']: future_symbol_js = 'ES'
    ...
    for fund in cfg.get('funds', []):
        ...
        if code == '161226':
            fund_home_row, fund_detail_page, fund_global_date = generate_fund_data(fund, data_processor, html_generator, futures_data, futures_history_df, is_index_table=False, gold_calibration=gold_calibration, oil_calibration=oil_calibration, basic_df=basic_df)
            if fund_detail_page:
        ...
        if category == '指数':
            ...
            fund_home_row_main, fund_detail_page, fund_global_date = generate_fund_data(fund, data_processor, html_generator, futures_data, futures_history_df, is_index_table=False, gold_calibration=gold_calibration, oil_calibration=oil_calibration, basic_df=basic_df)
            fund_home_row_index, _, _ = generate_fund_data(fund, data_processor, html_generator, futures_data, futures_history_df, is_index_table=True, gold_calibration=gold_calibration, oil_calibration=oil_calibration, basic_df=basic_df)
        ...
        <script>
            ...
            window.calcFutureSandbox = function(code) {
                try {
                    ...
                    // 同步提取并展示当前期货实盘价供参考
                    var futSym = baseData.futureSymbol;
                    var futPriceEl = futSym === 'GC' ? document.getElementById('gc-price') : document.getElementById('cl-price');
            ...
            window.calcPureFutureSandbox = function(code) {
                try {
                    ...
                    // 即使baseFuturePrice为0，也尝试计算纯期货估值
                    var exactChange = 0;
                    if (baseData.baseFuturePrice > 0) {
                        exactChange = futPrice / baseData.baseFuturePrice;
                    } else if (baseData.category === '黄金' || baseData.category === '原油' || baseData.category === '指数') {
                        // 对于主要类别，使用当前期货价格作为参考
                        exactChange = 1; // 临时解决方案，实际应该使用基准日的期货结算价
                    }
            ...
            window.calculateRealTimeValues = function() {
                ...
                var filteredRows = Array.from(allRows).filter(row => {
                    var isHomePage = !!row.closest('#page-home');
                ...
                filteredRows.forEach(function(row) {
                    ...
                    if (cells.length >= 9) { // 指数表9列，大宗表11列
                        ...
                        if (code === '162411') { canCalculate = (xopPrice > 0); }
                        else if (code === '161127') { canCalculate = (xbiPrice > 0); }
                        else if (category === '纯ETF') { 
                            if (code === '162411') { canCalculate = (xopPrice > 0); }
                            else if (code === '161127') { canCalculate = (xbiPrice > 0); }
                        }
                        else if (category === '黄金') { canCalculate = (gldPrice > 0); }
                        else if (category === '原油') { canCalculate = (usoPrice > 0); }
                        else if (code === '164701') { canCalculate = (gldPrice > 0 || slvPrice > 0); }
                        else if (category === '指数') { canCalculate = (spyPrice > 0 || qqqPrice > 0); }
                ...
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === 4 && xhr.status === 200) {
                        try {
                            ...
                            var gcPrice = data.GC ? data.GC.price : 0;
                            var gcChangePercent = data.GC ? data.GC.change_percent : 0;
                            var clPrice = data.CL ? data.CL.price : 0;
                            var clChangePercent = data.CL ? data.CL.change_percent : 0;
            ...
            window.updateFuturesTableColumns = function(gcPrice, clPrice, nqPrice, esPrice) {
                ...
                var fxChange = todayExchangeRate / baseData.baseExchangeRate;
                    ...
                    // 列3: 纯期货
                    // 即使baseFuturePrice为0，也尝试计算纯期货映射值
                    var exactChange = 0;
                    if (baseData.baseFuturePrice > 0) {
                        exactChange = futPrice / baseData.baseFuturePrice;
                    } else {
        ...
        <script>
            ...
            async function refreshAdminStatus() {
                try {
                    ...
                    ['011', '012', '013', 'calib'].forEach(task => {
                        const currentStatus = data[task] ? data[task].status : '未知';
                        setAdminStatus(task, currentStatus, data[task] ? data[task].last_run : '未运行');
                        
                        // 自动刷新逻辑：如果任务从 running 变为 success，自动刷新本页
                        if (prevTaskStatus[task] === 'running' && currentStatus === 'success') {
                            setTimeout(() => { window.location.reload(); }, 1500); // 延迟1.5秒确保后台文件写入彻底完成
                        }
                        prevTaskStatus[task] = currentStatus;
                    });
                    try {
    ...
    def get_prev_close(symbol):
        ...
        try:
            ...
            import os
            # 检查基础数据文件
            basic_data_path = os.path.join(os.path.dirname(__file__), 'data', 'GLD_USO_basic_data.csv')
    ...
    final_html += '                <button class="tab-button" onclick="switchTab(5)" style="background: var(--secondary-light); color: var(--secondary-dark); border: none; padding: 10px 20px; border-radius: 6px 6px 0 0; cursor: pointer; font-weight: bold; font-size: 14px; font-family: var(--font-sans); margin-left: auto;">⚙️ 静态数据维护</button>\n'
    final_html += '            </div>\n'
    ...
    final_html += '                    <div style="text-align: center; font-size: 16px; font-weight: bold; color: #555; margin-bottom: 20px;">数据维护控制面板</div>\n'
    final_html += '                    <div style="display: flex; gap: 30px; justify-content: center;">\n'
    final_html += '                        <div style="width: 150px; background: #eef6ff; border: 1px solid #cfe3ff; border-radius: 8px; padding: 15px; display:flex; flex-direction:column; justify-content: center; gap: 8px; box-shadow: var(--shadow-sm);">\n'
    final_html += '                            <div style="font-weight: bold; color: #1e4fa3; font-size: 18px; text-align: center;">A</div>\n'
    final_html += '                            <div style="font-size: 13px; color: #555; text-align:center; margin-bottom: 5px;">配置中心</div>\n'
    final_html += '                            <button class="admin-btn" style="background:#2f6fed; color:#fff; padding:6px 12px; font-size:13px; align-self: center; border-radius:4px; border:none; cursor:pointer;" onclick="openConfig()">打开面板</button>\n'
    final_html += '                            <div style="font-size: 11px; color: #555; text-align:center; margin-top: 5px;">状态: <b id="admin-lof00-status">未检测</b></div>\n'
    final_html += '                        </div>\n'
    final_html += '                        <div style="width: 180px; background: #fff6e6; border: 1px solid #ffd59e; border-radius: 8px; padding: 15px; display:flex; flex-direction:column; justify-content: center; gap: 8px; box-shadow: var(--shadow-sm);">\n'
    final_html += '                            <div style="font-weight: bold; color: #b25a00; font-size: 18px; text-align: center;">B</div>\n'
    final_html += '                            <div style="font-size: 13px; color: #555; text-align:center; margin-bottom: 5px;">基础数据(011)</div>\n'
    final_html += '                            <button class="admin-btn primary" style="background:#ff9f1a; color:#fff; padding: 6px 12px; font-size: 13px; align-self: center; border-radius:4px; border:none; cursor:pointer;" onclick="runAdminTask(\'011\')">获取API数据</button>\n'
    final_html += '                            <div style="font-size: 11px; color: #555; text-align:center; margin-top: 5px;"><b id="admin-011-status">未知</b></div>\n'
    final_html += '                            <div style="font-size: 11px; color: #666; text-align:center;" id="admin-011-time">未运行</div>\n'
    final_html += '                            <button class="admin-btn" style="padding: 4px 8px; font-size: 11px; align-self: center; margin-top: 5px; border-radius:4px; cursor:pointer;" onclick="openAdminLog(\'011\')">查看日志</button>\n'
    final_html += '                        </div>\n'
    final_html += '                        <div style="width: 180px; background: #ffeef1; border: 1px solid #f4c3cf; border-radius: 8px; padding: 15px; display:flex; flex-direction:column; justify-content: center; gap: 8px; box-shadow: var(--shadow-sm);">\n'
    final_html += '                            <div style="font-weight: bold; color: #1f7a4a; font-size: 18px; text-align: center;">C</div>\n'
    final_html += '                            <div style="font-size: 13px; color: #555; text-align:center; margin-bottom: 5px;">LOF数据表(012)</div>\n'
    final_html += '                            <button class="admin-btn primary" style="background:#f08aa5; color:#fff; padding: 6px 12px; font-size: 13px; align-self: center; border-radius:4px; border:none; cursor:pointer;" onclick="runAdminTask(\'012\')">计算LOF新值</button>\n'
    final_html += '                            <div style="font-size: 11px; color: #555; text-align:center; margin-top: 5px;"><b id="admin-012-status">未知</b></div>\n'
    final_html += '                            <div style="font-size: 11px; color: #666; text-align:center;" id="admin-012-time">未运行</div>\n'
    final_html += '                            <button class="admin-btn" style="padding: 4px 8px; font-size: 11px; align-self: center; margin-top: 5px; border-radius:4px; cursor:pointer;" onclick="openAdminLog(\'012\')">查看日志</button>\n'
    final_html += '                        </div>\n'
    final_html += '                    </div>\n'
