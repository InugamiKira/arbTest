...
def generate(futures_data=None, ib_data=None):
    ...
    for fund in cfg.get('funds', []):
        ...
        if base_date and base_nav:
            ...
            if base_row is not None:
                settle_col = f"{future_symbol_js}_settle" if future_symbol_js else None
                if settle_col and settle_col in base_row:
                    val = base_row.get(settle_col)
                    if pd.notna(val) and val != '无' and val != '':
                        base_future_price = float(val)
                if base_future_price <= 0 and '期货结算价' in base_row:
                    val = base_row.get('期货结算价')
                    if pd.notna(val) and val != '无' and val != '':
                        base_future_price = float(val)
    ...
    for fund in cfg.get('funds', []):
        ...
        if code == '161226':
            fund_home_row, fund_detail_page, fund_global_date = generate_fund_data(fund, data_processor, html_generator, futures_data, futures_history_df, is_index_table=False, calibrations=calibrations, global_er=global_er)
            if fund_detail_page:
        ...
        if category == '指数':
            ...
            fund_home_row_main, fund_detail_page, fund_global_date = generate_fund_data(fund, data_processor, html_generator, futures_data, futures_history_df, is_index_table=False, calibrations=calibrations, global_er=global_er)
            fund_home_row_index, _, _ = generate_fund_data(fund, data_processor, html_generator, futures_data, futures_history_df, is_index_table=True, calibrations=calibrations, global_er=global_er)
