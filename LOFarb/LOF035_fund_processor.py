...
def generate_fund_data(fund, data_processor, html_generator, futures_data, futures_history_df=None, is_index_table=False, calibrations=None, global_er=7.0):
    """处理单个基金的数据"""
    code = fund.get('code', '')
                ...
                if category == '黄金' and 'GC' in futures_data:
                    ...
                    if future_price > 0 and nav_home > 0:
                        ...
                        if base_date is not None:
                            ...
                            settle_col = f"{future_symbol}_settle" if future_symbol else "CL_settle"
                            if settle_col in df_idx.columns:
                                val = df_idx.loc[base_date].get(settle_col)
                                if pd.notna(val) and val != '无' and val != '':
                                    base_future_price = float(val)
                    ...
                    if future_price > 0 and nav_home > 0:
                        ...
                        # 提取基准日期货收盘价，供纯期货和期货校准双轨使用
                        base_future_price = 0.0
                        if base_date is not None:
                            base_date_str = base_date.strftime('%Y-%m-%d') if isinstance(base_date, pd.Timestamp) else str(base_date)[:10]
                            
                            settle_col = f"{future_symbol}_settle" if future_symbol else None
                            if settle_col and settle_col in df_idx.columns:
                                val = df_idx.loc[base_date].get(settle_col)
                                if pd.notna(val) and val != '无' and val != '':
                                    base_future_price = float(val)
                            elif '期货结算价' in df_idx.columns:
